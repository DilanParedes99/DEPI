        ...
        <div class="row">
            <div class="col-xl-6 col-12 offset-xl-3">
                <div class="search_results">
                    ...
                    include 'load/loadallarticles.php';
                    ?>
                    <input type="hidden" id="s-words" value='<?= $_GET['searchwords'] ?>'>
                </div>
                <div class="pagination pagination-sm ">
                    <?php
                    $sqlcount = "SELECT COUNT(articulos.idDoc) as num FROM docs INNER JOIN articulos ON articulos.idDoc=docs.idDoc" ;
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            ...
                            <li class="page-item <?= $backvalue ?>">
                                <a class="page-link" href="showallarticles.php?page=<?= ($pagina-1) ?>" aria-label="Previous">
                            ...
                            for($i = $pagerange;$i < $pagerange + 10 && $i <= $pags;$i++){
                                if($pagina%10 != 0 && $pagina/10 >= 1 && $i == $pagerange )
                                    echo '<li class="page-item "><a class="page-link" href="showallarticles.php?page='.($i-1).' ">'. ($i-1).'</a></li>';
                                $pagvalue = ($i == $pagina) ? "active" : "";      
                                echo '<li class="page-item '.$pagvalue.'"><a class="page-link" href="showallarticles.php?page='.$i.' ">'. $i.'</a></li>';
                            } ?>
                            <?php
                            if($pagina == $pagerange + 9)
                                echo '<li class="page-item"><a class="page-link" href="showallarticles.php?page='.($pagerange+10).'">'.($pagerange+10).'</a></li>';
                            $pagerangemax = floor($pags/10)*10 + 1;
                            if($pagerange !=  $pagerangemax){
                                ?>
                                <li class="page-item"><a class="page-link" href="#">...</a></li>
                                <li class="page-item"><a class="page-link" href="showallarticles.php?page=<?= $pags ?>"><?=$pags?></a></li>
                            <?php } ?>
                            <li class="page-item">
                                <a class="page-link" href="showallarticles.php?page=<?= ($pagina+1) ?>" aria-label="Next">
