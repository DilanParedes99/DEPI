        ...
        <div class="row">
            <div class="col-xl-6 col-12 offset-xl-3">
                <div class="search_results">
                    ...
                    include 'load/loadallthesis.php';
                ...
                <?php
                mysqli_set_charset($conn,'utf8' );
                    $sqlcount = "SELECT COUNT(tesis.idDoc) as num FROM docs INNER JOIN tesis ON tesis.idDoc=docs.idDoc" ;
                ...
                ?>
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            ...
                            <a class="page-link" href="showallthesis.php?page=<?= ($pagina-1) ?>" aria-label="Previous">
                            ...
                            <?php
                                for($i = $pagerange;$i < $pagerange + 10 && $i <= $pags;$i++){
                                    if($pagina%10 != 0 && $pagina/10 >= 1 && $i == $pagerange )
                                        echo '<li class="page-item "><a class="page-link" href="showallthesis.php?page='.($i-1).' ">'. ($i-1).'</a></li>';
                                    $pagvalue = ($i == $pagina) ? "active" : "";      
                                    echo '<li class="page-item '.$pagvalue.'"><a class="page-link" href="showallthesis.php?page='.$i.' ">'. $i.'</a></li>';
                                } ?>
                                <?php
                                if($pagina == $pagerange + 9)
                                    echo '<li class="page-item"><a class="page-link" href="showallthesis.php?page='.($pagerange+10).'">'.($pagerange+10).'</a></li>';
                                $pagerangemax = floor($pags/10)*10 + 1;
                                if($pagerange !=  $pagerangemax){
                                ?>
                                    <li class="page-item"><a class="page-link" href="#">...</a></li>
                                    <li class="page-item"><a class="page-link" href="showallthesis.php?page=<?= $pags ?>"><?=$pags?></a></li>
                                <?php } ?>
                            <li class="page-item">
                            <a class="page-link" href="showallthesis.php?page=<?= ($pagina+1) ?>" aria-label="Next">
